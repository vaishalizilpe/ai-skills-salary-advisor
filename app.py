...
<div class="hero">
    ...
    identifies skill gaps, and uses Claude AI to give you a brutally honest assessment of where you stand 
    and exactly what to do next.</p>
    <div class="hero-stats">
        <div class="hero-stat">
            <span class="number">20+</span>
            <span class="label">Live jobs analyzed</span>
        </div>
        <div class="hero-stat">
            <span class="number">26</span>
            <span class="label">Skills tracked</span>
        </div>
        <div class="hero-stat">
            <span class="number">Claude AI</span>
            <span class="label">Powering advice</span>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="how-it-works">
    ...
    <div class="steps">
        <div class="step">
            ...
            <div class="step-text">Fetches live job postings from Adzuna API</div>
        </div>
        <div class="step">
            <div class="step-num">2</div>
            <div class="step-text">Extracts and ranks in-demand skills from real descriptions</div>
        </div>
        <div class="step">
            <div class="step-num">3</div>
            <div class="step-text">Compares market demand against your skill set</div>
        </div>
        <div class="step">
            <div class="step-num">4</div>
            <div class="step-text">Claude AI gives personalized, direct career advice</div>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
st.markdown('<div class="input-hint">List everything you know, comma separated. Be honest — this only helps if it\'s accurate.</div>', unsafe_allow_html=True)
your_skills_input = st.text_area("", placeholder="e.g. SQL, Python, dbt, Snowflake, Airflow, Tableau, GCP", label_visibility="collapsed", height=90)
...
if st.button("▶  Analyze My Market Position", type="primary", use_container_width=True):
    ...
    if not job_title or not your_skills_input:
        st.warning("Fill in both fields to get your analysis.")
        ...
        <div class="results-header">
            ✓ Analyzed {total_jobs} live job postings for <strong>{job_title}</strong> · {len(skill_count)} skills detected
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            if missing:
                tags = "".join([f'<span class="tag-missing">{s} · {c}</span>' for s, c in missing[:6]])
                st.markdown(f'<div class="tag-container">{tags}</div>', unsafe_allow_html=True)
            else:
